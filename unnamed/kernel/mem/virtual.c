...
void removemap(uint64_t virtual)
{
	...
	curlevel = phymem_read64( (curlevel & ~0xFFF)   + (((virtual >> 30) % 512) * 8));  // PDPTE
	//DEBUG: dprintf("PDPTE: %x\n",curlevel);
	curlevel = phymem_read64( (curlevel & ~0xFFF)   + (((virtual >> 21) % 512) * 8));	// PDE
	//DEBUG: dprintf("PDE: %x\n",curlevel);
	//page 	 = phymem_read64( (curlevel & ~0xFFF)   + (((virtual >> 12) % 512) * 8));	// PTE
	//DEBUG: dprintf("PTE: %x\n",page);
	...
	phymem_write64( (curlevel&~0xFFF)   + (((virtual >> 12) % 512) * 8), 0 ); 	
}
...
int addmap(uint64_t physical, uint64_t virtual, unsigned int flags)
{
	...
	phymem_write64( (parent + indicies[0] * 8) , 
					physical | flags | 1 );
...
// Gets the 4k physical page that a virtual address maps to
